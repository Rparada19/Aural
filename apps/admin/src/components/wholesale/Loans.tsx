...
export function LoanLight({ dueOn }: { dueOn: string }) {
  ...
  const [cls, dot, label] =
    d < 0 ? ['wsale-bad', 'wsale-dot-bad', `${Math.abs(d)}d vencido`]
    : d === 0 ? ['wsale-bad', 'wsale-dot-bad', 'Vence hoy']
    : d <= 3 ? ['wsale-warn', 'wsale-dot-warn', `${d}d`]
    : ['wsale-good', 'wsale-dot-good', `${d}d`];
  return (
    <span className={`text-[11px] font-medium whitespace-nowrap w-20 text-right ${cls}`}>
    ...
    <div>
      <div className="flex items-baseline justify-between gap-4 mb-4">
        ...
        {active.filter((l) => daysLeft(l.due_on) < 0).length > 0 && (
          <span className="wsale-bad text-sm font-semibold">
      ...
      <div className="space-y-3">
        {active.length === 0 && (
          <p className="text-[var(--ink-soft)] text-sm">No hay equipos prestados.</p>
        )}

        {active.map((l) => (
          <div key={l.id} className="border border-[var(--rule)] rounded-[3px] p-4">
            ...
            <div className="flex flex-wrap gap-1 mt-3">
              {l.serials.map((s) => (
                <span key={s} className="text-xs font-mono bg-[var(--paper)] rounded px-2 py-1">{s}</span>
              ))}
            </div>
            ...
            {l.notes && <p className="text-[var(--ink-soft)] text-xs mt-1 italic">{l.notes}</p>}
            ...
            {selling === l.id ? (
              ...
              <div className="flex gap-3 mt-3 text-sm">
                ...
                <button
                  ...
                  disabled={busy === l.id}
                  className="text-[var(--ink-soft)] hover:text-[var(--ink)] disabled:opacity-50"
                >
                  Devuelto
                </button>
                <button
                  ...
                  disabled={busy === l.id}
                  className="text-[var(--ink-soft)] hover:wsale-bad disabled:opacity-50 ml-auto"
                >
                  Borrar
                </button>
              </div>
      ...
      {closed.length > 0 && (
        <details className="mt-5">
          ...
          <ul className="mt-3 space-y-2">
            {closed.map((l) => (
              <li key={l.id} className="flex items-center gap-3 text-sm border-b border-[var(--rule)] pb-2">
                <span className="flex-1 truncate">
                  {showClient && `${clientName.get(l.client_id) ?? ''} · `}
                  {l.serials.join(', ')}
                </span>
                <span className={`text-xs ${l.status === 'sold' ? 'wsale-good font-semibold' : 'text-[var(--ink-soft)]'}`}>
...
function SellForm({
  ...
  return (
    <form onSubmit={submit} className="mt-4 pt-3 border-t border-[var(--rule)] space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="Fecha de venta">
          <input type="date" required value={form.sold_on}
            onChange={(e) => setForm({ ...form, sold_on: e.target.value })} className="wsale-input" />
        </Field>
        <Field label="Factura">
          <input value={form.invoice_number}
            onChange={(e) => setForm({ ...form, invoice_number: e.target.value })} className="wsale-input" />
        </Field>
        <Field label="Precio de lista" hint="Por unidad.">
          <input type="number" required min="0" step="1000" value={form.list_price}
            onChange={(e) => setForm({ ...form, list_price: e.target.value })} className="wsale-input" />
        </Field>
        <Field label="Descuento %">
          <input type="number" min="0" max="100" step="0.5" value={form.discount_percent}
            onChange={(e) => setForm({ ...form, discount_percent: e.target.value })} className="wsale-input" />
        </Field>
      </div>
      <Field label="Campaña" hint="Opcional.">
        <input value={form.campaign_name}
          onChange={(e) => setForm({ ...form, campaign_name: e.target.value })} className="wsale-input" />
      </Field>
      {net > 0 && (
        <p className="text-sm">
    ...
    <form onSubmit={submit} className="border border-[var(--rule)] rounded-[3px] p-4 space-y-3">
      <Field label="Cliente">
        <select required value={form.client_id}
          onChange={(e) => setForm({ ...form, client_id: e.target.value })} className="wsale-input">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="Fecha del préstamo">
          <input type="date" required value={form.loaned_on}
            onChange={(e) => setForm({ ...form, loaned_on: e.target.value })} className="wsale-input" />
        </Field>
        <Field label="Devolver antes de" hint={`${LOAN_DAYS} días de plazo.`}>
          <input value={due} readOnly className={`${inputClass} bg-[var(--paper)] text-[var(--ink-soft)]`} />
        </Field>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <Field label="Plataforma">
          <select value={form.platform}
            onChange={(e) => setForm({ ...form, platform: e.target.value })} className="wsale-input">
            <option value="">—</option>
        ...
        <Field label="Tecnología">
          <select value={form.tech_level}
            onChange={(e) => setForm({ ...form, tech_level: e.target.value })} className="wsale-input">
            <option value="">—</option>
        ...
        <Field label="Formato">
          <select value={form.style}
            onChange={(e) => setForm({ ...form, style: e.target.value })} className="wsale-input">
      ...
      <div className="grid grid-cols-3 gap-3">
        <Field label="Unidades">
          <input type="number" min="1" required value={form.units}
            onChange={(e) => setForm({ ...form, units: e.target.value })} className="wsale-input" />
        </Field>
        <Field label="Adaptación">
          <select value={form.binaural}
            onChange={(e) => setForm({ ...form, binaural: e.target.value })} className="wsale-input">
        ...
        <Field label="Alimentación">
          <select value={form.power}
            onChange={(e) => setForm({ ...form, power: e.target.value })} className="wsale-input">
      ...
      <Field label="Seriales" hint="Uno por línea, o separados por coma. Es lo que hay que recuperar.">
        <textarea
          ...
          className="mt-1 w-full rounded-[2px] border border-[var(--rule)] p-2 text-sm font-mono outline-none focus:border-[var(--accent)]"
        />
      </Field>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="Paciente" hint="Opcional.">
          <input value={form.patient_name}
            onChange={(e) => setForm({ ...form, patient_name: e.target.value })} className="wsale-input" />
        </Field>
        <Field label="Nota">
          <input value={form.notes}
            onChange={(e) => setForm({ ...form, notes: e.target.value })} className="wsale-input" />
        </Field>
      </div>

      {error && <p className="wsale-bad text-sm">{error}</p>}
      <div className="flex gap-2">
        <button type="submit" disabled={saving}
          className="wsale-btn">
